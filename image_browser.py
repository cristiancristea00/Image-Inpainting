...
class ImageBrowser:
    ...
    def __init__(self, image_processor: ImageProcessor, path: Path = __DEFAULT_PATH, batch_size: int = 1,
                 should_crop: bool = False) -> None:
        """
        Args:
    ...
    def __load_image(self, filepath: Path | str) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        """
        ...
        Returns:
            tuple[tf.Tensor, tf.Tensor, tf.Tensor]: The masked image, the original image and the mask
        """
        ...
        masked, mask = self.image_processor.apply_mask(image)
    ...
    def __get_masked_tuple(self) -> tf.data.Dataset:
        ...
        return originals.prefetch(tf.data.AUTOTUNE)
    ...
    def __get_masked_pair(self) -> tf.data.Dataset:
        ...
        originals = self.__get_originals(Category.TEST)
        masked = originals.map(lambda masked, image, _: (masked, image), num_parallel_calls=tf.data.AUTOTUNE)
        return masked.prefetch(tf.data.AUTOTUNE)

    def __get_inpaint_transformer(self, inpaint_method: InpaintingMethod) -> Callable[
        [tf.Tensor, tf.Tensor, tf.Tensor], tuple[tf.Tensor, tf.Tensor]]:
        """
    ...
    def get_model_inpainted(self, model: tf.keras.Model) -> tf.data.Dataset:
        ...
        inpainted_dataset = normalized.map(lambda masked, original: (model(tf.expand_dims(masked, axis=0)), original),
